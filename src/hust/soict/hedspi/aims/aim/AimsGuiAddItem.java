...
public class AimsGuiAddItem extends JFrame {
...
private static final long serialVersionUID = 1L;
    ...
    public AimsGuiAddItem(Aims aims, DefaultListModel<String> itemList, int orderId, JTextArea txtTotalCost, JTextArea txtMessage) {
        ...
        btnAddTrack.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                // create new CD
                compactDisc cd = new compactDisc(aims.getOrders().get(orderId).getItemsOrdered().size(),
                        txtTitle.getText(),
                        txtCategory.getText(),
                        txtDirector.getText(),
                        Float.parseFloat(txtCost.getText()),
                        txtArtist.getText());
                new AimsGuiAddTrack(cd, Integer.parseInt(txtNumOfTrack.getText())); // open Aims add track gui
                trackInputted = true;
            }
        });
        ...
        btnAdd.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (Float.valueOf(txtCost.getText()) <= 0) {
                    warning(true, "Please enter a positive number for length!");
                    return;
                }
                
                Media newMedia = null;
                
                // check other fields are filled: ok -> add
                if (rdbtnDVD.isSelected()) {
                    ...
                    // check authors name; authors name: separated by ';'
                ...
                // update the item list & total cost & message
                if (newMedia != null) {
                    // update the item list
                    StringBuffer stringBuffer = new StringBuffer();
                  
                    stringBuffer.append(newMedia.getId() + " - ");
                    if (newMedia instanceof DigitalVideoDisc) {
                        stringBuffer.append("DVD - ");
                        stringBuffer.append(newMedia.getTitle() + " - ");
                        stringBuffer.append(newMedia.getCategory() + " - ");
                        stringBuffer.append(((DigitalVideoDisc)newMedia).getDirector() + " - ");
                        stringBuffer.append(((DigitalVideoDisc)newMedia).getLength() + " - ");
                        stringBuffer.append(newMedia.getCost());   
                    } else if (newMedia instanceof Book) {
                        stringBuffer.append("Book - ");
                        stringBuffer.append(newMedia.getTitle() + " - ");
                        stringBuffer.append(newMedia.getCategory() + " - ");
                        stringBuffer.append(((Book)newMedia).getAuthors().get(0));
                        for (int j = 1; j < ((Book)newMedia).getAuthors().size(); j++) {
                            stringBuffer.append(", " + ((Book)newMedia).getAuthors().get(j));    
                        }
                        stringBuffer.append(" - " + newMedia.getCost());    
                    } else { // CD
                        ...
                        stringBuffer.append(newMedia.getTitle() + " - ");
                        stringBuffer.append(newMedia.getCategory() + " - ");
                        stringBuffer.append(((compactDisc)newMedia).getArtist());
                        stringBuffer.append(" - " + newMedia.getCost());    
                    }
                    ...
                    // update message
                    int currentOrderSize = aims.getOrders().get(orderId).getItemsOrdered().size();
                    txtMessage.setText("Item " + newMedia.getTitle() + " has been added to Order!\n" +
                                         "The number of items in order now is " + currentOrderSize + ".\n");
                    if (currentOrderSize >= Order.MAX_NUMBER_ORDERED) {
                        txtMessage.append("The Order is full! Lets purchase your Order!");
                    } else if (currentOrderSize >= Order.PURCHASE_REMIND) {
                        txtMessage.append("The Order is almost full!");
                    }
                }
                
                // close frame
                thisFrame.dispose();
                new AimsGuiAfterAdd(newMedia);
            }
        });
    ...
    private void error(JTextArea txtMessage) {
